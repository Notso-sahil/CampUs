...
export default function Auth() {
  ...
  const handleGoogleSignIn = async (emailHint?: string) => {
    ...
    try {
      ...
      if (emailHint) {
        customProvider.setCustomParameters({ login_hint: emailHint });
      } else if (lastUsedEmail) {
      ...
      if (result.user?.email) {
        localStorage.setItem("last_used_email", result.user.email);
      }
    ...
    <div className="min-h-screen flex bg-white overflow-hidden">
      ...
      <div className="flex-1 flex flex-col px-8 py-8 bg-white relative">
        ...
        <div className="w-full max-w-sm space-y-7 mx-auto my-auto">
          ...
          {lastUsedEmail && (
            <button
              onClick={() => handleGoogleSignIn(lastUsedEmail)}
              disabled={loading}
              ...
              style={{ background: loading ? "#3b6fd4" : "#2563EB" }}
            >
              {loading ? (
                <Loader2 className="h-5 w-5 animate-spin" />
              ) : (
                <>
                  Continue as {lastUsedEmail}
          ...
          <button
            ...
            disabled={loading}
            className={`w-full flex items-center justify-center gap-3 h-12 px-6 rounded-xl font-semibold text-sm transition-all duration-200 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed ${lastUsedEmail ? 'bg-secondary/50 text-gray-700 hover:bg-secondary border border-border' : 'text-white shadow-md hover:shadow-lg'}`}
            style={{
              background: !lastUsedEmail ? (loading ? "#3b6fd4" : "#2563EB") : undefined,
            }}
          >
            {loading ? (
              <Loader2 className="h-5 w-5 animate-spin" />
              ...
              <>
                ...
                {lastUsedEmail ? "Use a different account" : "Continue with Google"}
                {!lastUsedEmail && <ArrowRight className="h-4 w-4 ml-auto" />}
              </>
            )}
          </button>
